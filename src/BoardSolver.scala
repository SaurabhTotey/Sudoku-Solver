...
object BoardSolver {
    ...
    def solveBoard(initialBoard: Board): Array[Board] = {
        ...
        var currentBoard: Board = if ((0 until 9).exists(i => initialBoard.row(i).count(num => num == 0) == 1)) {
            ...
            val outerBlockNumber = (0 until 9).filter(i => initialBoard.block(i % 3, i / 3).values.flatten.count(num => num == 0) == 1)(0)
            val innerBlockNumber = (0 until 9).filter(i => initialBoard.block(outerBlockNumber % 3, outerBlockNumber / 3).value(i % 3, i / 3) == 1)(0)
            initialBoard.alterValue((outerBlockNumber % 3) * 3 + innerBlockNumber % 3, outerBlockNumber / 3 * 3 + innerBlockNumber / 3, (1 to 9).filterNot(num => initialBoard.block(outerBlockNumber % 3, outerBlockNumber / 3).values.flatten.contains(num))(0))
        } else {
            backtrackVal += 1
