...
def solveBoard(initialBoard: Board): Array[Board] = {
    ...
    var backtrackR = -1
    var backtrackC = -1
    var currentBoard: Board = if ((0 until 9).map(i => 9 - initialBoard.row(i).filterNot(num => num == 0).length).contains(1)) {
        val rowIndex = (0 until 9).filter(i => 9 - initialBoard.row(i).filterNot(num => num == 0).length == 1)(0)
    ...
    } else if ((0 until 9).map(i => 9 - initialBoard.column(i).filterNot(num => num == 0).length).contains(1)) {
        val columnIndex = (0 until 9).filter(i => 9 - initialBoard.column(i).filterNot(num => num == 0).length == 1)(0)
